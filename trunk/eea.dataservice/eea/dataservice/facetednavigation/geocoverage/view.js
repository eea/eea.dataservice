...
Faceted.SelectWidget.prototype = {
  ...
  count: function(){
    ...
    query.cid = this.wid;
    query.sequence = jQuery.map(this.elements, function(element){
  ...
  count_update: function(data){
    ...
    jQuery(options).each(function(){
      ...
      var value = data[key];
      value = value ? value : 0;
      var option_txt = option.text();
      option_txt = option_txt.replace(/\s\(\d+\)/, '');
      option_txt += ' (' + value + ')';
...
var initializeSelectWidget = function(evt){
